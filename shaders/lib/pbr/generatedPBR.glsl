void generateIPBR(inout vec4 albedo, in vec3 worldPos, in vec3 viewPos, inout vec2 lightmap, inout float emission, inout float smoothness, inout float metalness, inout float subsurface) {
    ...
    //material == 2 are thin/transparent blocks (redstone wire, rails, pressure plates, carpets, etc.)
    //excluded so they don't get wet-puddle reflections
    if (emission < 0.001 && subsurface < 0.1 && material != 2) {
