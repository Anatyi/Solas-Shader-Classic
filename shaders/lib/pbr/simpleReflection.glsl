...
void getReflection(inout vec4 color, in vec3 viewPos, in vec3 normal, in float fresnel, in float smoothness) {
	...
	//Only sample the reflection buffer when the ray actually hit geometry
	//(reflectPos.z < 1.0 : not the far plane, depthtex0 < 1.0 : has geometry, border > 0.001 : inside screen)
	if (reflectPos.z < 1.0 - 1e-5 && border > 0.001 && texture2D(depthtex0, reflectPos.xy).r < 1.0) {
		reflection = texture2DLod(colortex0, reflectPos.xy + lod * refOffsets[0] / vec2(viewWidth, viewHeight), lod);
