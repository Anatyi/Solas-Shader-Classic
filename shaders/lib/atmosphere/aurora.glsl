//Aurora 3.7 Style (ported from Solas Shader 3.7) - volumetric aurora system
//Active only with AURORA_3_7 (default off). Depends on: AURORA_ALTITUDE, KARMAN_LINE,
//AURORA_BRIGHTNESS, worldDay, isSnowy, moonVisibility, wetness, caveFactor, noise3_7.
#ifdef AURORA_3_7
...
float getAuroraSample(vec2 coord, float kpIndex, float pulse, float longPulse, float auroraAltitudeMult, float auroraCameraY) {
	...
	float f = texture2D(noise3_7, flowUV).r * 2.0 - 1.0;
	...
	float sheet = texture2D(noise3_7, vec2(distortedCoord.x * 1.25, distortedCoord.y * 0.5 + frameTimeCounter * 0.0025)).r;
	      sheet *= sheet * sheet;

	//High frequency noise "rays"
	float rays = texture2D(noise3_7, vec2(distortedCoord.x * 5.0, distortedCoord.y * 2.0) + vec2(-frameTimeCounter * 0.0015, frameTimeCounter * 0.0025)).r;
	...
	if (visibility > 0.01) {
		...
		for (int i = 0; i < sampleCount; i++, rayPos += rayIncrement, sampleTotalLength += rayLength) {
			...
			vec3 auroraCoord = rayPos;
			     ...
			     auroraCoord *= 0.05 / AURORA_SCALE;   //Debug: band scale (larger = wider bands)

			vec2 coord = auroraCoord.xz + vec2(AURORA_OFFSET, 0.0);   //Debug: noise offset
			...
			if (auroraDistribution > 0.0) {
				...
				vec3 auroraColor = fmix(lowColor, upColor, colorMixer) * exp2(-4.0 * heightStep / AURORA_RADIAL);   //Debug: radial depth
