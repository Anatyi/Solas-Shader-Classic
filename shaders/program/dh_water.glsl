...
void main() {
	...
	if (mat == 10001) {
		#ifdef VANILLA_WATER
		//DH pass has no MC water texture bound - keep the vertex color, just use the vanilla alpha
		#else
	...
	if (mat != 10031) {
		...
		//Only apply the water planar reflection to actual water here. In this DH pass only DH_BLOCK_WATER
		//is tagged mat==10001; other blocks (e.g. redstone wire, mat==0) would get an unnatural mirror.
		if (mat == 10001) {
		...
		//Same restriction for the sun specular highlight
		if (mat == 10001) {
